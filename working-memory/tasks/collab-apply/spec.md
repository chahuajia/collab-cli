...
**状态**：待 review（A10 / W8 的「规格」阶段，未写测试、未写实现）
**作者**：codex（AI 提议）
**日期**：2026-09-16
...
1. **写盘后 validate 失败要不要回滚？** 建议：不回滚（见反面）。
2. **`--commit` 默认值**：建议默认关闭，必须显式传。
3. **是否允许 `apply` 写 `inbox/`**：建议允许（bundle 可先归档到 inbox）。
4. **`base_commit` 要不要强制校验**：建议第一版**只记录、不阻塞**（阻塞会造成"必须 rebase 才能落盘"的摩擦）。
