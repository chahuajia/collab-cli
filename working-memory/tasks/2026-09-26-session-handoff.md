...
> ⚠️ **本文写于 2026-09-26 下午；当天傍晚又改了三件事，先看这三条再读下文**：
> ① 发布版本改判 **`0.6.0`**（parse 契约是 breaking）→ `2026-09-26-version-gap.md` + `RELEASE.md`；
> ② 生成物里的安装范围改为从 `package.json` **派生**（原先四处手写 `@^0.5`）；
> ③ **两个 CI 从来没跑起来**（`npm ci` + pnpm 锁文件）→ 已改 pnpm，见 `2026-09-26-ci-never-ran.md`。
> 下文里的 `0.5.1` / `776 测试` 是**当时**的值（现为 0.6.0 / 804），别当现状引用。
