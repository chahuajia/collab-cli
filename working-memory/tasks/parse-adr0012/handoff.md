...
> ⚠️ **本文是任务级的**（只讲 parse 契约变更）。同一天的**会话级**交接在
> `tasks/2026-09-26-session-handoff.md` —— 那一份覆盖 6 个关切（parse / init /
> 发布卫生 / `src/cli` DDD / `scripts` TS / 三轮可用性排查），并列了**未完成清单**。
> 从那一份开始读，只在需要 parse 细节时回到本文。
