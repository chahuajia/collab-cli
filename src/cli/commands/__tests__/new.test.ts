...
import { toFail, toSucceed } from "@/cli/commands/__tests__/testHelpers";
...
async function setupWorkspace(opts: {
    ...
    if (!opts.skipGit) {
        ...
        if (!opts.skipEmail) {
            await execa('git', ['config', '--local', 'user.email', opts.gitEmail ?? 'test@example.com'], {
                cwd: testRoot,
    ...
    if (!opts.skipCollab) {
        for (const sub of ['skills', 'workflows', 'agreements', 'patterns', 'meta/decision-records']) {
...
describe('collab new', () => {
    ...
    describe('基础场景', () => {
        ...
        it('created file has valid frontmatter', async () => {
            ...
            const content = await readFile(path.join(collabDir, 'skills/S30.md'), 'utf8');
    ...
    describe('ID 自动生成', () => {
        ...
        it('generates S6 when S1-S5 exist (E12)', async () => {
            for (let i = 1; i <= 5; i++) {
                await writeFile(path.join(collabDir, `skills/S${i}.md`), '');
            }
            await toSucceed(
              ["new", "skill"],
              testRoot,
            ...
            expect(existsSync(path.join(collabDir, 'skills/S6.md'))).toBe(true);
        });
        ...
        it('generates ADR-0001 for first ADR (E4)', async () => {
           await toSucceed(
            ...
            expect(existsSync(path.join(collabDir, 'meta/decision-records/ADR-0001.md'))).toBe(true);
        });

        it('generates ADR-0002 with zero-padding (E13)', async () => {
            await writeFile(path.join(collabDir, 'meta/decision-records/ADR-0001.md'), '');
            await toSucceed(
              ["new", "adr"],
              testRoot,
              envOverrides,
            );
            expect(existsSync(path.join(collabDir, 'meta/decision-records/ADR-0002.md'))).toBe(true);
        });

        it('skips nested files when generating next id', async () => {
            await mkdir(path.join(collabDir, 'skills/advanced'), { recursive: true });
            await writeFile(path.join(collabDir, 'skills/advanced/S12.md'), '');
            await toSucceed(
              ["new", "skill"],
              testRoot,
              envOverrides,
    ...
    describe('Pattern 类型', () => {
        ...
        it('creates pattern with explicit id', async () => {
            ...
            expect(existsSync(path.join(collabDir, 'patterns/my-pattern.md'))).toBe(true);
        });
    });
